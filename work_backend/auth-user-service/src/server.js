...
function normalizeSamlProfile(profile) {
  ...
  const oid = firstValue(
    ...
    profile?.['urn:oid:1.3.6.1.4.1.5643.10.0.1'],
    profile?.['http://schemas.microsoft.com/identity/claims/objectidentifier'],
...
async function createOrRefreshSession(user) {
  ...
  await redis.set(`session:${sid}`, JSON.stringify(session), 'EX', 60 * 60 * 8);
...
app.get('/auth/metadata-mapping', function(req, res) {
  res.json({
    expectedMappings: {
      ...
      name: ['urn:oid:2.5.4.3', 'urn:oid:2.16.840.1.113730.3.1.241', 'urn:oid:2.5.4.42 + urn:oid:2.5.4.4']
    }
  });
});

app.post('/auth/saml/callback', passport.authenticate('saml', { session: false, failureRedirect: buildFrontendUrl(UNAUTHORIZED_PATH) }), async function(req, res, next) {
  try {
